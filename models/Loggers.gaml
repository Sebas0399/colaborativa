...
global {
	...
	action registerLogFile(string filename) {
		if traditionalScenario = true {
			filenames[filename] <- './../data/TraditionalScenario/' + string(logDate, 'yyyy-MM-dd hh.mm.ss','en') + '/' + filename + '.csv';
		} else {
			filenames[filename] <- './../data/AutonomousScenario/' + string(logDate, 'yyyy-MM-dd hh.mm.ss','en') + '/' + filename + '.csv';
		}
	}
	...
	action logForSetUp (list<string> parameters) {
		loop param over: parameters {
			if traditionalScenario = true {
				save (param) to: './../data/TraditionalScenario/' + string(logDate, 'yyyy-MM-dd hh.mm.ss','en') + '/' + 'setUp' + '.txt' type: "text" rewrite: false header: false;
			} else {
				save (param) to: './../data/AutonomousScenario/' + string(logDate, 'yyyy-MM-dd hh.mm.ss','en') + '/' + 'setUp' + '.txt' type: "text" rewrite: false header: false;
			}
		}
	}
