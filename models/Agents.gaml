		...
		list<geometry> segments_s <- p_total.segments;
			loop line over: segments_s {
				ask road(p_total agent_from_geometry line) { 
					if rain_level > max_s_trip {
						max_s_trip <- rain_level;
					}
				}
...
species road {
	...
	//The rain level
	float rain_level <-0.0001; 
	
	// probability for a agent to choose this road
	float proba_use <- rain_level/maxSnow;
	
	//TODO: review color map
	int colorValue <- int(255*(rain_level*0.1)) update: int(255*(rain_level*0.1));
	...
	reflex snow when: every(10#minute){
		rain_level <- rain_level + rainrate/6;
...
species package control: fsm skills: [moving] {
	...
	state firstmile {
		...
		transition to: awaiting_car when: choice = 1 and location = target{mode <- 1;}
		exit {
			if packageEventLog {ask logger{do logExitState;}}
		}
